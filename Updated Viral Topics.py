...
# 4) Keywords input box (original niche finder keywords)
keywords_input = st.text_area(
    "Paste Keywords (comma-separated):",
    value=""
)

# NEW: 5) Additional user keywords for better targeting
user_keywords_input = st.text_area(
    "Add Your Own Keywords for Better Results (comma-separated, optional):",
    value="",
    help="Add relevant keywords to refine search. These will be combined with each niche keyword above."
)

# Convert pasted text into list
keywords = [k.strip() for k in keywords_input.split(",") if k.strip()]
user_keywords = [k.strip() for k in user_keywords_input.split(",") if k.strip()]
    ...
    try:
        ...
        if not keywords:
            ...
            for keyword in keywords:
                # Combine niche keyword with user keywords
                if user_keywords:
                    combined_keywords = [f"{keyword} {user_kw}" for user_kw in user_keywords]
                    combined_keywords.insert(0, keyword)  # Also search original keyword
                else:
                    combined_keywords = [keyword]
                
                for search_term in combined_keywords:
                    ...
                    if "items" not in data or not data["items"]:
                        st.warning(f"No videos found for: {search_term}")
                        continue
                    ...
                    if not video_ids:
                        st.warning(f"Skipping: {search_term} due to missing video data.")
                        continue
                    ...
                    if "items" not in stats_data or not stats_data["items"]:
                        st.warning(f"Failed to fetch video statistics for: {search_term}")
                        continue
                    ...
                    if "items" not in channel_data or not channel_data["items"]:
                        st.warning(f"Failed to fetch channel statistics for: {search_term}")
                        continue
